document.getElementById('btn-rectangle').addEventListener('click', function () {
    const newRectangleCalculateW = getValueById('rectangle-w');

    const newRectangleCalculateL = getValueById('rectangle-l');

    const newElement = newRectangleCalculateW * newRectangleCalculateL;
    setAns('Rectangle', newElement);
    if (newRectangleCalculateW < 0 ) {
        alert('get positive number')
    }
    else if (newRectangleCalculateL < 0) {
        alert('get positive number')
    }
})
