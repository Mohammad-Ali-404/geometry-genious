document.getElementById('btn-parallelogram').addEventListener('click', function () {
    const newParallelogramCalculateB = getValueById('parallelogram-b');

    const newParallelogramCalculateH = getValueById('parallelogram-h');

    const newElement = newParallelogramCalculateB * newParallelogramCalculateH;
    setAns('parallelogram', newElement);
    if (newParallelogramCalculateB < 0 ) {
        alert('get positive number')
    }
    else if (newParallelogramCalculateH < 0) {
        alert('get positive number')
    }
})
